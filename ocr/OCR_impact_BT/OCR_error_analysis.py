...
def prep_data_for_BT(lang_code):
    DATA = "UDHR"
...
def extract_OCR_errors(GT_input, OCRed_input):
    ...
    for i in tqdm.tqdm(range(len(input_txt_ocr))):
        groundtruth_split_input_part = unicodedata.normalize('NFC', input_txt[i].replace('\u200d', '').replace('\u200c', ''))
        OCRed_split_output_part = unicodedata.normalize('NFC', input_txt_ocr[i].replace('\u200d', '').replace('\u200c', ''))
...
def measure_OCR_error_rate(OCRed_input, GT_input):
    with open(OCRed_input, encoding="utf8") as f:
        input_txt_ocr = f.readlines()
    with open(GT_input, encoding="utf8") as f:
        input_txt = f.readlines()
    ...
    for i in tqdm.tqdm(range(0,10000,1000)):
        ...
        if groundtruth_split_input_part and OCRed_split_output_part:
            cer = ed.eval(OCRed_split_output_part, groundtruth_split_input_part) / float(len(groundtruth_split_input_part))
            wer = ed.eval(OCRed_split_output_part.split(), groundtruth_split_input_part.split()) / float(len(groundtruth_split_input_part.split()))
...
def read_monolingual_data():
    threshold = 1.03
    # threshold = 0 #npi 100k -> 40k
    max_lines = 10000
    # max_lines = 20000
    max_words = 40
    ...
    # for lang_code in ["npi", "tam", "khm", "lao", "amh", "jpn", "pus"]:
    for lang_code in ["npi"]:
        print(lang_code)
        mono = []
        if lang_code in dict_codes_cc100:
            ...
            with open("Data/MONOLINGUAL/WikiMatrix." + dict_codes_wikimatrix[lang_code] + ".tsv", encoding="utf-8") as csvfile:
                reader = csv.reader(csvfile, delimiter='\t', quoting=csv.QUOTE_NONE)
                for row in reader:
                    if float(row[0]) > threshold and len(row) == 3 and row[2] and len(row[2].split()) <= max_words and (row[1] not in row[2]) and (row[2] not in row[1]):
                        mono.append(row[2])
...
def write_error_rate(all_mono_ocred, all_mono, path_output, size):
...
def introduce_errors_in_monolingual(all_error_types, lang_code, error_rate, size):
    ...
    all_mono = " ".join(mono)
    if lang_code != 'jpn':
        if os.path.isfile(path_output + "/replace/" + lang_code + "_mono.txt") and sum(1 for _ in open(path_output + "/replace/" + lang_code + "_mono.txt")) == data_size: 
            ...
            write_error_rate(all_mono_ocred, all_mono, path_output, size)
    ...
    if lang_code != 'jpn':
        if not all_error_types['delete']:
            ...
            if os.path.isfile(path_output + "/delete/" + lang_code + "_mono.txt") and sum(1 for line in open(path_output + "/delete/" + lang_code + "_mono.txt")) == data_size: 
    ...
    all_mono = " ".join(mono)
    if os.path.isfile(path_output + "/insert/" + lang_code + "_mono.txt") and sum(1 for line in open(path_output + "/insert/" + lang_code + "_mono.txt")) == data_size: 
...
def read_finetuning_results(max_error_rate, output_file, size):
    if 'flores' in output_file:
        dict_finetune_clean_results = {'amh':2.5,'nep':0.1,'lao':6.7,'khm':9.8,'pus':10.0,'jpn':22.8,'tam':3.4}
    else: #finetuned ocred data
        dict_finetune_clean_results = {'amh':3.4,'nep':0.4,'lao':8.6,'khm':10.0,'pus':9.9,'jpn':22.9,'tam':3.1}
    ...
    unique_combinations = list(product(['khm', 'pus', 'lao', 'tam'], ['insert', 'delete', 'replace'])) + [('jpn', 'insert')]
    ...
    for i, data in enumerate(results):
        ...
        if i % 2 == 0:
            ...
            error_rate = int(error_rate) if ((int(error_rate) % 2) == 0) else int(error_rate) + 1 #round to the nearest even
        else:
            ...
            for j in range(len(csv_out['Language'])):
                if (((csv_out['Language'][j] == lang) and (csv_out['Error type'][j] == error_type)) and (csv_out['CER'][j] == error_rate)):
                    duplicate = True
                    break
            if not duplicate:
                csv_out['gt_score'].append(dict_finetune_clean_results[lang])
                ...
                csv_out['Language'].append(lang)
                csv_out['Error type'].append(error_type)
                # if (lang, error_type) not in dict_lang_error_type.keys():
                #     dict_lang_error_type[(lang, error_type)] = []
                # error_rate_even = int(error_rate) if ((int(error_rate) % 2) == 0) else int(error_rate) + 1 #round to the nearest even
                # dict_lang_error_type[(lang, error_type)].append((error_rate, error_rate_even))

    df = pd.DataFrame.from_dict(csv_out)
    sorted_df = df.sort_values(by=["CER"], ascending=True) #sort by CER
    pd.DataFrame(sorted_df).reset_index().to_csv('Data/backtranslation/results/BT/' + size + "/" + output_file + '.csv', header=True, index=False)


if __name__ == "__main__":
    #introduce OCR errors
    for lang_code in ["pus", "khm", "lao", "tam", "jpn"]:
        prep_data_for_BT(lang_code)
        all_error_types = extract_OCR_errors("Data/UDHR/UDHR_" + lang_code + "_GT.txt", "Data/UDHR/UDHR_" + lang_code + "_OCR.txt")
        for error_rate in range(1, 32, 2):
            introduce_errors_in_monolingual(all_error_types, lang_code, error_rate)
    
    # # verify, measure OCR error rate
    # for lang_code in ["pus", "khm", "lao", "tam", "jpn"]:
    #     for error_rate in range(1, 22, 2):
    #         for operation in ['delete', 'insert','replace']:
    #             print(error_rate, operation)
    #             input_path = "Data/backtranslation/data_cer/" + str(error_rate) + "/" + operation + "/" + lang_code + "_mono.txt"
    #             output_path = "Data/MONOLINGUAL/10k/" + lang_code + "_mono.txt"
